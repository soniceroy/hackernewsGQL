...
func (r *mutationResolver) CreateLink(ctx context.Context, input model.NewLink) (*model.Link, error) {
	...
	link.User = user
	linkID := link.Save()
	return &model.Link{ID: strconv.FormatInt(linkID, 10), Title: link.Title, Address: link.Address, User: &model.User{ID: link.User.ID, Name: link.User.Username}}, nil
}
...
func (r *queryResolver) Links(ctx context.Context) ([]*model.Link, error) {
	...
	for _, link := range dbLinks {
		...
		linksResp = append(linksResp, &model.Link{ID: link.ID, Title: link.Title, Address: link.Address, User: graphqlUser})
	}
